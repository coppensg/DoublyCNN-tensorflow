...
class Model:
    def __init__(
        ...
        reshaped_inputs = tf.reshape(self.inputs, [-1, in_depth, height, width], name="input_formatted")
        ...
        for l in range(self.n_layers):
            ...
            # Pooling layer
                ...
                s = filter_shape[l][0]
                self.layers.append(tf.nn.max_pool(self.layers[-1], [1, s, s, 1], strides=[1, s, s, 1], padding='SAME', name='Pool_{}'.format(l)))
                self.layers.append(tf.nn.dropout(self.layers[-1], keep_prob=self.keep_prob, name='Dropout_{}'.format(l)))
            else:
                raise NotImplementedError
        ########################################################################################################
